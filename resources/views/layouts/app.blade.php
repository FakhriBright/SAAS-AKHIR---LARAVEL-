...
<head>
    ...
    <title>@yield('title', 'Catering Admin')</title>
    ...
    {{-- ✨ UI LIBRARIES: DataTables --}}
    <link rel="stylesheet" href="https://cdn.datatables.net/1.13.7/css/dataTables.bootstrap5.min.css">

    {{-- ✨ UI LIBRARIES: SweetAlert2 (Toast) --}}
...
<body>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                ...
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    {{-- Menu CRUD: Cuma Admin yang lihat --}}
                    @if(Auth::user()->level === 'admin')
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('pakets.*') ? 'active fw-bold' : '' }}" href="{{ route('pakets.index') }}">
                            <i class="bi bi-box-seam"></i> Paket
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('pelanggans.*') ? 'active fw-bold' : '' }}" href="{{ route('pelanggans.index') }}">
                            <i class="bi bi-people"></i> Pelanggan
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('pemesanans.*') ? 'active fw-bold' : '' }}" href="{{ route('pemesanans.index') }}">
                            <i class="bi bi-cart"></i> Pemesanan
                        </a>
                    </li>

                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle {{ request()->routeIs('jenis-pembayaran.*', 'detail-jenis-pembayaran.*') ? 'active fw-bold' : '' }}" href="#" role="button" data-bs-toggle="dropdown">
                            <i class="bi bi-credit-card"></i> Pembayaran
                        </a>
                        <ul class="dropdown-menu shadow border-0">
                            <li><a class="dropdown-item" href="{{ route('jenis-pembayaran.index') }}">Jenis Pembayaran</a></li>
                            <li><a class="dropdown-item" href="{{ route('detail-jenis-pembayaran.index') }}">Detail Pembayaran</a></li>
                        </ul>
                    </li>
                    @endif

                    {{-- Pengiriman: Admin & Kurir --}}
                    @if(in_array(Auth::user()->level, ['admin', 'kurir']))
                    <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('pengirimans.*') ? 'active fw-bold' : '' }}" href="{{ route('pengirimans.index') }}">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle d-flex align-items-center gap-2" href="#" role="button" data-bs-toggle="dropdown">
                            ...
                            <span class="d-none d-lg-inline">
                                {{ Auth::user()->name }}
                                <small class="text-white-50">({{ ucfirst(Auth::user()->level) }})</small>
                            </span>
                        </a>
    ...
    <footer class="bg-white text-center py-4 mt-5 border-top">
        <div class="container">
            <small class="text-muted">&copy; {{ date('Y') }} Catering App. Built with <i class="bi bi-heart-fill text-danger"></i> & Laravel.</small>
        </div>
    </footer>

    {{-- Scripts --}}
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
    {{-- ✨ UI LIBRARIES: jQuery & DataTables --}}
