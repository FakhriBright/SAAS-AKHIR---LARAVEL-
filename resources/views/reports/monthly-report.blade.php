...
<body>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-box">
            <h3>{{ number_format($totalPesanan > 0 ? $totalPendapatan / $totalPesanan : 0, 0, ',', '.') }}</h3>
    ...
    <div class="status-summary">
        <table>
            ...
            <tr>
                ...
                <td class="text-center">{{ $statusData['Menunggu Konfirmasi'] }}</td>
                <td class="text-center">{{ $totalPesanan > 0 ? round(($statusData['Menunggu Konfirmasi'] / $totalPesanan) * 100, 1) : 0 }}%</td>
            </tr>
            <tr>
                <td><span class="badge badge-info">Sedang Diproses</span></td>
                <td class="text-center">{{ $statusData['Sedang Diproses'] }}</td>
                <td class="text-center">{{ $totalPesanan > 0 ? round(($statusData['Sedang Diproses'] / $totalPesanan) * 100, 1) : 0 }}%</td>
            </tr>
            <tr>
                <td><span class="badge badge-secondary">Menunggu Kurir</span></td>
                <td class="text-center">{{ $statusData['Menunggu Kurir'] }}</td>
                <td class="text-center">{{ $totalPesanan > 0 ? round(($statusData['Menunggu Kurir'] / $totalPesanan) * 100, 1) : 0 }}%</td>
            </tr>
            <tr>
                <td><span class="badge badge-success">Selesai</span></td>
                <td class="text-center">{{ $statusData['Selesai'] }}</td>
                <td class="text-center">{{ $totalPesanan > 0 ? round(($statusData['Selesai'] / $totalPesanan) * 100, 1) : 0 }}%</td>
            </tr>
            <tr>
                <td><span class="badge badge-danger">Dibatalkan</span></td>
                <td class="text-center">{{ $statusData['Dibatalkan'] }}</td>
                <td class="text-center">{{ $totalPesanan > 0 ? round(($statusData['Dibatalkan'] / $totalPesanan) * 100, 1) : 0 }}%</td>
            </tr>
        </table>
    </div>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center">
                    ...
                    @endphp
                    <span class="badge badge-{{ $badgeClass }}">{{ $order->status_pesan }}</span>
                </td>
            </tr>
            @empty
            <tr>
