...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">No. Resi: <strong>{{ $pemesanan->no_resi }}</strong></p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('pemesanans.index') }}" class="btn btn-secondary">
                <i class="bi bi-arrow-left"></i> Kembali
            </a>
            <a href="{{ route('pemesanans.edit', $pemesanan->id) }}" class="btn btn-warning">
                <i class="bi bi-pencil"></i> Edit
            </a>
            <a href="{{ route('pemesanans.pdf', $pemesanan->id) }}" class="btn btn-danger" target="_blank">
                <i class="bi bi-file-earmark-pdf"></i> Cetak Invoice
            </a>
            <form action="{{ route('pemesanans.destroy', $pemesanan->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pemesanan ini?')">
                @csrf
                @method('DELETE')
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-4">
            <div class="card shadow-sm h-100 border-0">
                ...
                <div class="card-body">
                    <p class="mb-2"><strong>Nama:</strong> {{ $pemesanan->pelanggan->nama_pelanggan ?? '-' }}</p>
                    <p class="mb-2"><strong>Email:</strong> {{ $pemesanan->pelanggan->email ?? '-' }}</p>
                    <p class="mb-2"><strong>Telepon:</strong> {{ $pemesanan->pelanggan->telepon ?? '-' }}</p>
                    <p class="mb-0"><strong>Alamat:</strong><br>
                        {{ $pemesanan->pelanggan->alamat1 ?? '-' }}
                        @if($pemesanan->pelanggan->alamat2), {{ $pemesanan->pelanggan->alamat2 }}@endif
                        @if($pemesanan->pelanggan->alamat3), {{ $pemesanan->pelanggan->alamat3 }}@endif
                    </p>
                </div>
            </div>
        </div>

        {{-- Card Informasi Pembayaran & Status --}}
        <div class="col-md-6 mb-4">
            <div class="card shadow-sm h-100 border-0">
                ...
                <div class="card-body">
                    <p class="mb-2"><strong>Metode:</strong> {{ $pemesanan->jenisPembayaran->metode_pembayaran ?? '-' }}</p>
                    <p class="mb-2"><strong>Tanggal Pesan:</strong> {{ $pemesanan->tgl_pesan?->format('d/m/Y H:i') ?? '-' }}</p>
                    <p class="mb-2"><strong>Status:</strong>
                        ...
                        @else
                            <span class="badge bg-light text-dark">{{ $pemesanan->status_pesan ?? '-' }}</span>
                        @endif
                    </p>
                    <p class="mb-0"><strong>No. Resi:</strong> {{ $pemesanan->no_resi }}</p>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @if($detail->paket && $detail->paket->deskripsi)
                                    <br><small class="text-muted">{{ Str::limit($detail->paket->deskripsi, 60) }}</small>
                                @endif
                            </td>
