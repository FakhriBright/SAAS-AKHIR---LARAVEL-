...
<div class="container">
    ...
    <div class="row g-4">
        <div class="col-lg-8">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="d-flex align-items-center justify-content-between border-bottom pb-3 mb-3">
                        <div class="d-flex align-items-center gap-3 flex-grow-1">
                            <input type="checkbox" class="form-check-input cart-check" value="{{ $cart->id }}" data-subtotal="{{ $cart->subtotal }}">
                            <div class="flex-grow-1">
                                <h6 class="fw-bold mb-1">{{ $cart->paket->nama_paket }}</h6>
                                <p class="text-muted small mb-0">Rp {{ number_format($cart->paket->harga_paket, 0, ',', '.') }} / porsi</p>
                            </div>
                        </div>
                        <div class="d-flex align-items-center gap-2">
                            <form action="{{ route('customer.cart.update', $cart->id) }}" method="POST">
                                @csrf @method('PATCH')
                                <input type="hidden" name="jumlah" value="{{ max(1, $cart->jumlah - 1) }}">
                            ...
                            <form action="{{ route('customer.cart.update', $cart->id) }}" method="POST">
                                @csrf @method('PATCH')
                                <input type="hidden" name="jumlah" value="{{ $cart->jumlah + 1 }}">
                        ...
                        <form action="{{ route('customer.cart.remove', $cart->id) }}" method="POST" class="ms-3">
                            @csrf @method('DELETE')
            ...
            <a href="{{ route('customer.catalog') }}" class="btn btn-fk-outline mt-3"><i class="bi bi-arrow-left me-2"></i>Lanjut Belanja</a>
        </div>
        ...
        <div class="col-lg-4">
            <div class="card fk-card sticky-top" style="top: 90px;">
                <div class="card-body p-4">
                    ...
                    <button class="btn btn-fk-primary w-100 btn-lg" id="btn-checkout" disabled onclick="window.location.href='{{ route('customer.checkout') }}?cart_ids=' + getSelectedIds()">
    ...
    <div class="text-center py-5">
        ...
        <a href="{{ route('customer.catalog') }}" class="btn btn-fk-primary btn-lg px-4">Belanja Sekarang</a>
    </div>
    @endif
</div>
